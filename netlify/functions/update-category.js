...
exports.handler = async function (event) {
  ...
  try {
    ...
    const { id, nameAr, nameEn } = body;
    if (!id) return { statusCode: 400, body: JSON.stringify({ error: 'id required' }) };

    const pool = getPool();
    const res = await pool.query('UPDATE categories SET name_ar=$1, name_en=$2 WHERE id=$3 RETURNING id, name_ar, name_en, created_at', [nameAr || null, nameEn || null, id]);
